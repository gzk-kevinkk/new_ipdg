...
func_interface_N(FLOAT x, FLOAT y, FLOAT z, FLOAT *values)
{
	...
	{
		FLOAT Cx = Cos(2. * PI * x), Cy = Cos(2. * PI * y), Cz = Cos(2. * PI * z);
		values[0] = -1. * PI * Sin(2. * PI * x) * Cy * Cz;
		values[1] = -1. * PI * Cx * Sin(2. * PI * y) * Cz;
		values[2] = -1. * PI * Cx * Cy * Sin(2. * PI * z);
	}
}
...
int main(int argc, char *argv[])
{
	...
	phgOptionsPreset("-solver hypre -hypre_solver gmres -hypre_pc boomeramg -solver_rtol 1e-15 -solver_btol 1e-12");
#endif
...
#warning ****************** using 0 as initial solution!
	...
	phgSetupHalo(g, HALO_FACE);
		...
		phgDofSetDataByLambdaFunction(error, func_u_lambda);
